...
Although no one has proved it yet, it is thought that some numbers, like 196, never produce a palindrome. 
     ...
     every number below ten-thousand, it will either (i) become a palindrome in less than fifty iterations, or, 
...
NOTE: Wording was modified slightly on 24 April 2007 to emphasise the theoretical nature of Lychrel numbers.*/
    ...
    class Program
    {
        ...
        {
            int countLychrel = 0;
            for (long nn = 1; nn <= 10000; nn++ )
            {
                if (IsLychrel(new BigInteger(nn)))
                {
                    countLychrel++;
                }
            }
            Util.ShowResult( countLychrel);
        }
